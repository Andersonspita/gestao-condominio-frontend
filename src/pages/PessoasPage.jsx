...
const PessoasPage = () => {
    ...
    return (
        <div className="page-container">
            <div className="page-header"><h1>Gestão de Pessoas</h1><button onClick={handleAddNew}>Adicionar Pessoa</button></div>
                        ...
                        <div className="roles-list">
                            {papeisUsuario.length > 0 ? papeisUsuario.map((papel, index) => (
                                <div key={index} className="role-item">
                                    {papel.tipo === 'morador' ? (<span><strong>Morador</strong> ({formatStatus(papel.morTipoRelacionamento)}) na Unid. {papel.unidade.uniNumero} (Cond. {papel.unidade.condominio.conNome})</span>) : (<span><strong>{formatStatus(papel.uscPapel)}</strong> no Cond. {papel.condominio.conNome}</span>)}
            ...
            {isLoading ? <p>Carregando...</p> : (<div className="table-container"><table><thead><tr><th>Nome</th><th>Email</th><th>CPF/CNPJ</th><th>Status</th><th>Ações</th></tr></thead><tbody>{pessoas.map(pessoa => (<tr key={pessoa.pesCod}><td>{pessoa.pesNome}<RoleBadges roles={pessoa.roleNames} /></td><td>{pessoa.pesEmail}</td><td>{maskCpfCnpj(pessoa.pesCpfCnpj)}</td><td>{pessoa.pesAtivo ? 'Ativo' : 'Inativo'}</td><td className="actions-cell"><button onClick={() => handleEdit(pessoa)} className="btn-edit">Editar</button><button onClick={() => handleOpenPasswordModal(pessoa)} className="btn-password">Alterar Senha</button><button onClick={() => handleToggleAtivo(pessoa)} className={pessoa.pesAtivo ? "btn-inactive" : "btn-active"}>{pessoa.pesAtivo ? 'Inativar' : 'Ativar'}</button></td></tr>))}</tbody></table></div>)}
        </div>
    );
};
